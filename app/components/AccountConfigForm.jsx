...
const AccountConfigForm = ({ t, customView, fields, dirty, error, submit, submitting }) => (
  <div class={'account-form' + (error ? ' error' : '')}>
    {fields.folderPath &&
      <div>
        <h3>{t('data_connect location')}</h3>
        <p>{t('data_connect location desc')}</p>
        ...
        <a href={folderLink(fields.folderPath.value, fields.folderPath.folders)} target='_blank'>
          {t('data_connect location button')}
        </a>
      </div>
    }
    {fields.calendar &&
      <div>
        <h3>{t('data_connect calendar')}</h3>
        <p>{t('data_connect calendar desc')}</p>
        <Field label={false} {...fields.calendar} />
      </div>
    }
    <h3>{t('data_connect frequency')}</h3>
    <p>{t('data_connect frequency desc')}</p>
